...
import { client } from "../libs/client-cloudwatchlogs.js";
...
const run = async () => {
  const command = new PutSubscriptionFilterCommand({
    ...
    destinationArn: process.env.CLOUDWATCH_LOGS_DESTINATION_ARN || "123",

    // A name for the filter.
    filterName: process.env.CLOUDWATCH_LOGS_FILTER_NAME || "TESTFILTER",

    // A filter pattern for subscribing to a filtered stream of log events.
    // https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/FilterAndPatternSyntax.html
    filterPattern: process.env.CLOUDWATCH_LOGS_FILTER_PATTERN || "TESTPATTERN",

    // The name of the log group. Messages in this group matching the filter pattern
    // will be sent to the destination ARN.
    logGroupName: process.env.CLOUDWATCH_LOGS_LOG_GROUP || "TESTLOGGROUP",
  });
